...
Shader "GAP/IceShader01" {
    ...
    SubShader {
        ...
        Pass {
            ...
            struct IceAttributes { float4 positionOS : POSITION; float2 uv : TEXCOORD0; float3 normalOS : NORMAL; half4 color : COLOR; };
            struct IceVaryings { float4 positionCS : SV_POSITION; float2 uv : TEXCOORD0; float3 normalWS : TEXCOORD1; float3 positionWS : TEXCOORD2; half4 color : COLOR; };
            IceVaryings vert(IceAttributes input)
            {
                IceVaryings output;
            ...
            half4 frag(IceVaryings input) : SV_Target
            {
                half4 mainSample = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, input.uv);
                float3 normalWS = normalize(input.normalWS);
                float3 viewDir = normalize(GetWorldSpaceViewDir(input.positionWS));
                half fresnel = pow(saturate(1.0 - dot(normalWS, viewDir)), _FresnelArea) * _FresnelPower;
                half3 fresnelRgb = _FresnelColor.rgb * fresnel;
                half3 rgb = mainSample.rgb * _Color.rgb * _ColorBrightness * fresnelRgb * input.color.rgb;
