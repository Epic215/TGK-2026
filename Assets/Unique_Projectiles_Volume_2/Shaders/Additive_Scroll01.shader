...
Shader "GAP/AdditiveDistortionScroll" {
    ...
    SubShader {
        ...
        Pass {
            ...
            #pragma target 3.0
            #include "GAP_URP_ParticlesInclude.hlsl"
            ...
            CBUFFER_END
            TEXTURE2D(_MainTexutre); SAMPLER(sampler_MainTexutre);
            ...
            GAPVaryings vert(GAPAttributes v) { return GAPVert(v); }
            half4 frag(GAPVaryings i, float facing : VFACE) : SV_Target
            {
                half3 rgb = GAPDistortionScrollAdditive(i, _TintColor, _ColorMultiplier, _MainTextureUSpeed, _MainTextureVSpeed,
                    _DistortMainTexture, _GradientPower, _GradientUSpeed, _GradientVSpeed, _NoiseAmount,
                    _DistortionUSpeed, _DistortionVSpeed, _EdgeSoftness, _DoubleSided, facing,
                    _MainTexutre_ST, _Gradient_ST, _Distortion_ST, _Mask_ST, _ColorRamp_ST,
                    TEXTURE2D_ARGS(_MainTexutre, sampler_MainTexutre), TEXTURE2D_ARGS(_Gradient, sampler_Gradient),
                    TEXTURE2D_ARGS(_Distortion, sampler_Distortion), TEXTURE2D_ARGS(_Mask, sampler_Mask),
                    TEXTURE2D_ARGS(_ColorRamp, sampler_ColorRamp));
                return half4(rgb, 1);
            }
            ENDHLSL
        }
