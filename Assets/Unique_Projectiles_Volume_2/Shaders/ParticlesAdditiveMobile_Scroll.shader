...
Shader "GAP/ParticlesAdditiveMobile_Scroll"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag(GAPVaryings i) : SV_Target
			{
				half4 c = GAPSampleMobileScroll(i, _Color, _MainTexTiling, _MainTexSpeed, _DistortionAmount,
					_DistortionTiling, _DistortionSpeed, _DissolveAmount, _DissolveTiling, _DissolveSpeed,
					TEXTURE2D_ARGS(_Mask, sampler_Mask), TEXTURE2D_ARGS(_MainTex, sampler_MainTex),
					TEXTURE2D_ARGS(_DistortionTexture, sampler_DistortionTexture),
					TEXTURE2D_ARGS(_DissolveTexture, sampler_DissolveTexture));
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutput o )
		{
			...
			float2 uv_TexCoord42 = i.uv_texcoord * _DistortionTiling;
			float2 panner25 = ( 1.0 * _Time.y * _DistortionSpeed + uv_TexCoord42);
			float4 lerpResult17 = lerp( float4( i.uv_texcoord, 0.0 , 0.0 ) , tex2D( _DistortionTexture, panner25 ) , _DistortionAmount);
			float2 uv_TexCoord45 = i.uv_texcoord * _DissolveTiling;
			float2 panner30 = ( 1.0 * _Time.y * _DissolveSpeed + uv_TexCoord45);
